...
% the key here is that we use two metrics:
%   1. "eventness" \label{eq:eventness} to assess how much a time interval is related to real-world events (we're still working on getting the perfect metric here)
%   2.  document relevancy for an event: $f(t_d,t)\,\E[\epsilon_{dt}]$ \label{eq:docrecovery}.  For a given time interval, we order all documents by this metric, and the top ones are most relevant to the time interval (or event)

To detect events, we consider the proportion of a document $d$ about an event at time interval $t$, or
% TODO: I don't like using p
\begin{equation}
p_{dt} = \frac{f(t_d, t) \E[\epsilon_{d,t}]}{\E[\zeta_d] + \sum_{i=1}^T f(t_d, i) \E[\epsilon_{d,i}] + \sum_{k=1}^K \E[\theta_{d,k}] }
\end{equation}
and take a weighted average of these proportions to measure ``eventness'' at time $t$:
\begin{equation}
m_t = \frac{1}{\sum_{d} f(t_d, t)} \sum_{d} p_{dt}.
\label{eq:eventness}
\end{equation}
This measure of ``eventness'' estimates the fraction of term
...
the most strongly-associated messages by computing an event relevancy metric
for each message $d$ and ordering the messages with this metric.
For event $t$ and document $d$, we define this metric as
\begin{equation}
m_{dt} = f(t_d, t)\,\E[\epsilon_{dt}].
\label{eq:docrecovery}
\end{equation}
