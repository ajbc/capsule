...
time. As described in \cref{sec:additional_results}, we

We found that an exponential decay function, as in \cref{eq:f},
works well in practice.

% I think this paragraph should be moved to the model section.
As described in \Cref{sec:additional_results}, we assessed the
sensitivity of o\
ur model to different settings of event duration $\tau$ three
different decay f\
unctions $f$: exponential, linear, and step functions.  We found that
fitting C\
apsule with an exponential decay function, or
\begin{equation}
  f(i_d, t) =
  \begin{cases}
        0,                  & \text{if } t \le i_d < t + \tau\\
            \exp\left\{\frac{-(i_d - t)}{\tau/5}\right\},          &
            \text{otherwise,}
            \end{cases}
  \label{eq:f}
  \end{equation}
...
t)\,\E[\epsilon_{dt}]$ for each message $d$ and ordering the messages
