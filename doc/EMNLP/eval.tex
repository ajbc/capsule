...
list, provided to us by the History Lab at the University of Columbia,
of thirty-nine real-world events that took place between 1973 and
1978. Each event is present in at least one of six reputable
...
We treated this list as ``ground truth'' and assessed how well Capsule
and each of the baselines described in \cref{sec:detecting_events}
...
\end{equation}
where $L_T^{\textrm{method}}$ is the ranked list of time intervals
according to the method and $\textrm{rank}\left(e,
L_T^{\textrm{method}}\right)$ is the rank of the $e^{\textrm{th}}$
real-world event in $L_T^{\textrm{method}}$. Finally, we divided the
DCG by the ideal DCG---i.e., $\sum_{e=1}^{39}
\frac{1}{\log{\left(e\right)}}$---to obtain the normalized
DCG (nDCG). \Cref{table:cables:ndcg} shows that Capsule outperforms the
baselines.
...
\begin{tabular}{l c}
\toprule
\textbf{Method} & \textbf{normalized DCG} \\
\midrule
Capsule & 0.693 \\
