...
ggplot(dat, aes(x=reorder(method, value, FUN=max), y=value)) + geom_violin(alpha=0.2, color="white", fill="#156946") + geom_point(alpha=0.4) +
  ...
  coord_flip() +
  scale_x_discrete(labels=c("Random", "Event only (this paper)",  "tf-idf word deviation (Eq. 5)", "word count deviation (Eq. 4)","Capsule (this paper)")) +
  ylab("Event detection AUC") + xlab("")
