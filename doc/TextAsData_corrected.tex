...
%\footnote{We use an atypical parameterization of the Gamma with parameters mean $\mu$ and sparsity $\alpha$: \[\mbox{Gamma}(x \g \mu, \alpha) = \frac{\alpha^\alpha}{\Gamma(\alpha) \mu^\alpha} x^{(\alpha-1)}e^{-\alpha x / \mu}.\]  It is equivalent to the usual shape/scale parameterization where shape $k=\alpha$ and scale $\theta=\mu/\alpha$.}
\end{itemize}
...
%\log\mbox{Gamma}(x \g \mathcal{P}(a), \mathcal{P}(b)) & = 
%\mathcal{P}(a)\mathcal{P}(b)\log \mathcal{P}(b) - \log\Gamma \left(\mathcal{P}(a)\mathcal{P}(b)\right) +(\mathcal{P}(a)\mathcal{P}(b)-1)\log x - \mathcal{P}(b)x \\
...
%\frac{d}{d b}\log\mbox{Gamma}(x \g \mathcal{P}(a), \mathcal{P}(b)) &= 
%\mathcal{P}'(b) \left[ \mathcal{P}(a) (\log \mathcal{P}(b) + 1) - \mathcal{P}(a)\Psi \left(\mathcal{P}(a)\mathcal{P}(b)\right) + \mathcal{P}(a)\log x - x\right]
\end{align}
...
\While {change in validation likelihood $< \delta$}{
	...
	\For {each sample $s = 1, \dots, S$}{
		\For {each component $k$} {
			draw sample entity topics $\phi_{0k}[s] \sim \mbox{Gamma}(\mathcal{P}(\lambda^\phi_{k}))$ \;
			\BlankLine
			set $p^\phi_k[s] = \log p(\phi_{0k}[s] \g \alpha, \beta)$ \tcp*{see Eqn.~\ref{eq:gamma}}
			set $q^\phi_k[s] = \log q(\phi_{0k}[s] \g \lambda^\phi_k)$ \tcp*{Eqn.~\ref{eq:gamma} with params $\mathcal{P}(\lambda^{\phi}_k)$}
			set $g^\phi_k[s] = \nabla_{\lambda^\phi_{k}} \log q(\phi_{0k}[s] \g \lambda^\phi_{k})$ \tcp*{see Eqns.~\ref{eq:dgammaA}, \ref{eq:dgammaB}}
		}
		\For {each timestep $i$} {
			draw sample event occurance $\epsilon_i[s] \sim \mbox{Poisson}(\mathcal{P}(\lambda^\epsilon_i))$ \;
			\BlankLine
			set $p^\epsilon_i[s] = \log p(\epsilon_{i}[s] \g \eta)$ \tcp*{see Eqn.~\ref{eq:poisson}}
			set $q^\epsilon_i[s] = \log q(\epsilon_i[s] \g \lambda^\epsilon_i)$ \tcp*{Eqn.~\ref{eq:poisson} with param $\mathcal{P}(\lambda^{\epsilon}_i)$}
			set $g^\pi_{i}[s] = \nabla_{\lambda^\epsilon_{i}} \log q(\epsilon_{i}[s] \g \lambda^\epsilon_{i})$ \tcp*{see Eqn.~\ref{eq:dpoisson}}
			\BlankLine
			\If {$\epsilon_i[s] \neq 0$} {
				\For {each component $k$} {
					draw sample event topics $\pi_{ik}[s] \sim \mbox{Gamma}(\mathcal{P}(\lambda^\pi_{ik}))$ \;
					\BlankLine
					set $p^\pi_{ik}[s] = \log p(\pi_{ik}[s] \g \alpha_0, \beta_0)$ \tcp*{see Eqn.~\ref{eq:gamma}}
					set $q^\pi_{ik}[s] = \log q(\pi_{ik}[s] \g \lambda^\pi_{ik})$ \tcp*{Eqn.~\ref{eq:gamma} with params $\mathcal{P}(\lambda^{\pi}_{ik})$}
